...
def mag_to_flux(filter_name: str, mag: float) -> float:
    """
    Transform a magnitude to a physical flux (erg cm-2 s-1 Å-1).

    Parameters
    ----------
    filter_name : str
        Filter name. Filter must be specified in the filter_profiles folder.
    mag : float
        Filter magnitude.
    ...
    -------
    float
        Physical flux corresponding to filter magnitude.
    ...
    try:
        zp = filter_data_df.loc[filter_name, 'zp']
    except KeyError as e:
        raise KeyError(f'The chosen filter-name "{filter_name}" is not specified in the file '
                       f'"{filter_data_path}".'
                       'If you use a new filter, make sure to enter the name, zeropoint and filter wavelength in the '
                       '"filter_data.csv" file.') from e
    return 10 ** (0.4 * (zp - mag))
